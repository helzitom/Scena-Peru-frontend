...
export default function LoginForm() {
    ...
    async function manejarSubmit(e: React.FormEvent) {
        ...
        try {
            if (modo === "registro") {
                const usuario = await registrarUsuario({ email, password, tipo, ciudadId: 1, nombreDisplay });
                guardarSesion({ email: usuario.email, nombreDisplay: usuario.nombreDisplay, tipo: usuario.tipo });
                setEstado({ tipo: "ok", mensaje: "Cuenta creada. Entrando..." });
                router.push(destino);
            } else {
                await iniciarSesion({ email, password });
                guardarSesion({ email, nombreDisplay: email, tipo: "FAN" });
            ...
            setEstado({
                ...
                mensaje:
                    modo === "login"
                        ? "El backend todavía no tiene login implementado (falta JWT). Crea una cuenta para probar la app."
        ...
        <div className="w-full max-w-sm mx-auto">
            ...
            <form onSubmit={manejarSubmit} className="space-y-4">
                {modo === "registro" && (
                    <>
                        <div>
                            <label className="font-mono text-[11px] uppercase tracking-wide text-ink/50">Soy...</label>
